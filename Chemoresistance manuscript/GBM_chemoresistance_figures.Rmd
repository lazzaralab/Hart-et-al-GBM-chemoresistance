...
```{r}
##OLD DATA DO NOT USE
dat1 <- read_excel("../11-11-22 U87L TMZ carbo JNKi cell death results.xlsx")
colnames(dat1) <- colnames(dat1) %>% gsub("chemo", "treat", .)
dat1$inhib <- dat1$inhib %>% gsub("dmso", "DMSO", .) 

dat2 <- dat1 %>% 
  dplyr::select(inhib, treat, rep1, rep2, rep3) %>% 
  pivot_longer(-c(inhib, treat), values_to = "alive", names_to = "rep") %>% 
  mutate(dead = 100 - alive)

dat3 <- dat2 %>% group_by(inhib, treat) %>% 
  summarise(avg_dead = mean(dead), stdev = sd(dead))

dat3$inhib <- dat3$inhib %>% factor(c("DMSO", "JNKi"))
dat3$treat <- dat3$treat %>% gsub("CARB", "CARBO", .) %>% factor(c("DMSO", "TMZ", "CARBO"))

dat3 %>% ggplot(aes(x = inhib, y = avg_dead, fill = inhib))+
  geom_bar(stat = "identity", position ="dodge", color = "black")+
  geom_errorbar(aes(ymax = avg_dead+stdev, ymin = avg_dead-stdev), width = 0.3)+
  facet_wrap(~treat)+
  theme_cowplot(8)+
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)))+
  theme(legend.position = "none")+
  scale_fill_okabe_ito(order = c(9, 5))
ggsave("figure export/JNKi_cell_death.svg", width = 3, height = 2, units = "in")

for (t in unique(factor(dat2$treat))) {
  c1 <- dat2 %>% filter(inhib == "DMSO", treat == t) %>% pull(dead)
  c2 <- dat2 %>% filter(inhib == "JNKi", treat == t) %>% pull(dead)
  print(t)
  t.test(c1, c2) %>% print()
}
...
##JNKi cell death - 
```{r}
dat1 <- read_excel("../02-28-23 siJUN JNKi TMZ carbo cell death analysis.xlsx", range = "A9:D15")
dat1 <- dat1 %>% pivot_longer(cols = -c(inhib, treat), names_to = "rep", values_to = "alive") %>% 
  mutate(dead = 100-alive)

dat2 <- dat1 %>% group_by(inhib, treat) %>% 
  summarise(avg_dead = mean(dead), sd = sd(dead))
dat2$treat <- dat2$treat %>% factor(c("DMSO", "TMZ", "CARBO"))

dat2 %>% ggplot(aes(x = inhib, y = avg_dead, fill = inhib))+
  geom_bar(stat = "identity", position ="dodge", color = "black")+
  geom_errorbar(aes(ymax = avg_dead+sd, ymin = avg_dead-sd), width = 0.3)+
  facet_wrap(~treat)+
  theme_cowplot(8)+
  labs(x = element_blank(), y = "% dead")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)))+
...
##PTEN TMZ/CARBO qPCR
```{r}
dat1 <- read_excel("../11-01-22 U87L TMZ carbo RTqPCR PTEN.xlsx", sheet = "Sheet2")
dat2 <- dat1 %>% group_by(treat) %>% summarise(mean = mean(PTEN), sd = sd(PTEN))
dat2$treat <- dat2$treat %>% factor(c("DMSO", "TMZ", "CARBO"))

dat2 %>% ggplot(aes(x = treat, y = mean, fill = treat))+
  geom_bar(stat = "identity", position ="dodge", color = "black")+
  ...
  labs(title = "PTEN")+
  scale_fill_okabe_ito(order = c(9, 5, 3))
ggsave("figure export/PTEN_qPCR_TMZ-CARBO.svg", width = 1.5, height = 2, units = "in")

anova1 <- aov(PTEN ~ treat, data = dat1)
summary(anova1)
hsd1 <- TukeyHSD(anova1)
hsd1
```
...
##PTEN CARBO+MAPKi qPCR
```{r}
dat1 <- read_excel("../11-15-22 U87L carbo MAPKi Chk1i RTqPCR PTEN.xlsx", sheet = "Sheet2")

dat1 <- dat1 %>% group_by(treat) %>% summarise(mean = mean(PTEN), sd = sd(PTEN))
dat1$treat <- dat1$treat %>% gsub("\\+", "\n", .) %>% factor(c("DMSO", "CARBO", "CARBO\nJNKi", "CARBO\nMEKi", "CARBO\np38i"))

dat1 %>% ggplot(aes(x = treat, y = mean, fill = treat))+
  geom_bar(stat = "identity", position ="dodge", color = "black", aes(alpha = treat))+
  geom_errorbar(aes(ymax = mean+sd, ymin = mean-sd), width = 0.3)+
  theme_cowplot(8)+
  labs(x = element_blank(), y = "Normalized Fold Change")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)))+
  theme(legend.position = "none", plot.title = element_text(face = "italic", hjust = 0.5))+
  labs(title = "PTEN")+
  scale_fill_okabe_ito(order = c(9, 5, 5, 5, 5))+
  scale_alpha_manual(values = c(1, 1, 0.8, 0.6, 0.4))
ggsave("figure export/PTEN_qPCR_Carbo+MAPKi.svg", width = 2.5, height = 2, units = "in")
ggsave("figure export/PTEN_qPCR_Carbo+MAPKi.png", width = 2.5, height = 2, units = "in")
```
##PTEN siJUN qPCR
```{r}
dat1 <- read_excel("../12-09-22 U87L carbo cJun siRNA RTqPCR (PTEN cJun).xlsx", sheet = "Sheet2")

dat2 <- dat1 %>% mutate(siRNA = if_else(grepl("ctrl", treat), "Control \nsiRNA", "cJun \nsiRNA"),
                        drug = if_else(grepl("unt", treat), "Untreated", "CARBO"))
dat3 <- dat2 %>% group_by(siRNA, drug) %>% 
  summarise(mean = mean(PTEN), sd = sd(PTEN))

dat3$siRNA <- dat3$siRNA %>% factor(c("Control \nsiRNA", "cJun \nsiRNA"))
dat3$drug <- dat3$drug %>% factor(c("Untreated", "CARBO"))

dat3 %>% ggplot(aes(x = siRNA, y = mean, fill = siRNA))+
  geom_bar(stat = "identity", position ="dodge", color = "black")+
  geom_errorbar(aes(ymax = mean+sd, ymin = mean-sd), position = "dodge", width = 0.3)+
  theme_cowplot(8)+
  labs(x = element_blank(), y = "Normalized Fold Change")+
  scale_y_continuous(expand = expansion(mult = c(0, 0.3)))+
  theme(legend.position = "none", plot.title = element_text(face = "italic", hjust = 0.5))+
  ...
  scale_fill_okabe_ito(order = c(9, 5))+
  facet_wrap(~drug)
ggsave("figure export/PTEN_qPCR_cJun-si.svg", width = 2.5, height = 2, units = "in")
ggsave("figure export/PTEN_qPCR_cJun-si.png", width = 2.5, height = 2, units = "in")

anova1 <- aov(PTEN ~ siRNA+drug, data = dat2)
summary(anova1)
hsd1 <- TukeyHSD(anova1)
hsd1
