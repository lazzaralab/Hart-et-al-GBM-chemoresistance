...
```{r}
dat1 <- read_excel("../../03-13-24 U87L-MYC hypoxia TMZ cell death results.xlsx", range = "A1:F9") %>% 
  pivot_longer(cols = -c(O2, line, treat), values_to = "alive", names_to = "rep") %>% 
  mutate(dead = 100-alive)
dat1$O2 <- dat1$O2 %>% factor(c("21", "1"))
dat1$line <- dat1$line %>% factor(c("EV", "MYC"))
...
ggplot(dat1, aes(x = O2, y = dead, fill = O2))+
  ...
  theme(legend.position = "none")+
  scale_fill_okabe_ito(order = c(5,3))+
  facet_grid(cols = vars(line), rows = vars(treat))
